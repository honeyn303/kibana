...
* For an <<gemini-action-type,{gemini} connector>>, current support is for the Gemini models. Defaults to `gemini-1.5-pro`.
...
`xpack.actions.preconfigured.<connector-id>.secrets.ccredentialsJson`::
For an <<bedrock-action-type,{bedrock} connector>>, specifies the AWS access key for authentication.
